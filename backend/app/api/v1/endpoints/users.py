...
async def get_mentionable_users(
    ...
    if current_user.role == UserRole.BDC:
        ...
        if dealership_id:
            if accessible_ids is not None and dealership_id not in accessible_ids:
                return []
            scope_ids = [dealership_id]
        elif accessible_ids is None:
            org_id = await resolve_user_dealership_id(db, current_user)
            if not org_id:
                return []
            scope_ids = [org_id]
...
async def list_bdc_agents(
    ...
    if current_user.role == UserRole.SUPER_ADMIN:
        ...
        target_dealership = dealership_id or await resolve_user_dealership_id(db, current_user)
...
async def get_user_dealership_access(
    ...
    # Single-org BDC accounts often have an empty access table. Fall back only
    # when the BDC user is loading their own picker — not when Super Admin is
    # editing assignments.
    if not dealerships and current_user.role == UserRole.BDC:
        org_id = await resolve_user_dealership_id(db, target)
        if org_id:
            org_row = await db.execute(
                select(Dealership.id, Dealership.name).where(Dealership.id == org_id)
            )
            row = org_row.first()
            if row:
                dealerships = [DealershipAccessItem(id=row[0], name=row[1])]
        if not dealerships:
            ...
            dealerships = [
                DealershipAccessItem(id=row[0], name=row[1]) for row in all_rows.fetchall()
            ]
